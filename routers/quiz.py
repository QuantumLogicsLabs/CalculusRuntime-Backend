...
for the question bank and the module docstring below for the token design.
"""
...
from quiz_bank import get_quiz
...
async def submit_quiz(request: Request):
    ...
    original question index) — the frontend never needs to know original
    indices. Grading un-shuffles via `opt_perm`/`order` from the token and
    compares against quiz_bank.py, entirely server-side.
    """
    user_id = require_user(request)
    if not user_id:
        return err(401, "Not authenticated.")
